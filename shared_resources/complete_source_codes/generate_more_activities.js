...
const allAIs = db.prepare(`
  ...
  LIMIT 1800
`).all();
console.log(`   활동 예정 AI: ${allAIs.length}명`);
...
for (let i = 0; i < allAIs.length; i++) {
  ...
  for (let j = 0; j < activityCount; j++) {
    ...
    const content = `${ai.team_code} ${activityType} activity`;
    const target = `target_${Math.floor(Math.random() * 1000)}@example.com`;
  ...
  // 진행 상황 표시 (100명마다)
  if ((i + 1) % 100 === 0) {
    console.log(`   ${i + 1}/${allAIs.length} AI 완료 (${totalActivities}개 활동 생성)`);
  }
}
