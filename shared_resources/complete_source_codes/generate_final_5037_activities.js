...
for (let i = 0; i < mainAIs.length; i++) {
  const ai = mainAIs[i];
  // AI ID를 숫자로 변환 (ai_0001 -> 1)
  const aiIdStr = ai.id.replace('ai_', '');
  const aiIdNum = parseInt(aiIdStr) + 20000; // 중복 방지를 위해 20000 더하기
  
  allAIs.push({
    ai_id: aiIdNum,
    ai_name: ai.name || `AI_${aiIdNum}`,
    team_code: ai.team || 'GENERAL'
  });
}

console.log(`\n4️⃣ 최종 AI 목록: ${allAIs.length}명 (5,037명 목표)`);
...
for (let i = 0; i < allAIs.length; i++) {
  ...
  for (let j = 0; j < activityCount; j++) {
    ...
    const content = `${ai.team_code} ${activityType} - 5037명 전체 실시간 통신`;
  ...
  // 진행 상황 표시 (1000명마다)
  if ((i + 1) % 1000 === 0 || i + 1 === allAIs.length) {
